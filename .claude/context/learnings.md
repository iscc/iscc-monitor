...
| `internal/certificate` | `learnings/certificate.md` | realm-wide `GET /inclusion/{iscc_id}` cert; first caller of decode→resolve→ListHubs→SeqsForISCCID chain; fail-closed 200 verdicts + buffer-then-200; §1 gated on accepted-tree cap `seqs[0] < LastSize` + canonicalized `ISCC:`-prefixed lookup key |
