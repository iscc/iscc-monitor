...
| `.github/workflows/` (ci.yml + pages.yml + publish.yml) | `learnings/ci.md` | CI: one CGO_ENABLED=0 job inlining mise check + the notecheck oracle shell-out (pipefail/SIGPIPE caveat); Pages: verifier-site build→deploy, copies byte-pinned verify.wasm, artifact CNAME is a no-op under Actions (custom domain is a repo-settings step); Publish: GHCR build-push on develop, `:develop`+`:sha-<short>` tags, `github.sha` build-arg sidesteps empty-stamp, `.dockerignore` slashless globs only match context-root |
...
| `internal/dossier` + `store.ListHubs` §3/§4 | `learnings/dossier.md` | per-hub numbered trust-document `GET /<domain>` (§1–§5 + Exhibit/caution); honesty-gated §1–§4; §3 size/time decouple (frozen mismatch); §1 unconditional "resolved" vs unresolvable; masthead/overlay ports from dashboard |
...
| `internal/openapi` | `learnings/openapi.md` | hand-authored OpenAPI 3.1 contract (yaml source + byte-distinct json twin, go:embed served byte-verbatim); pure stdlib+yaml.v3 leaf; drift test gates PATHS ONLY (blind to params/media-types/responses — hand-check those vs the handler); machineProbes is hand-maintained ground truth; regenerate the json twin after any yaml edit; `/docs` is SSR → ssrExclusions, never the contract |
| `internal/docs` | (`learnings/web.md` + `openapi.md`) | tiny pure-stdlib SSR leaf serving `GET /docs` — the Stoplight Elements API reference mounting `<elements-api apiDescriptionUrl="/openapi.json">`; static (no per-request data), buffer-then-200, 405 non-GET; assets/no-CDN nuance in web.md, drift exclusion in openapi.md |
