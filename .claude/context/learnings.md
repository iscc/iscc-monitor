...
| `internal/ots` | `learnings/ots.md` | OpenTimestamps confirmed-check adapter; pure parse/classify (no I/O); NOT WASM-pure (keep out of WASM-shared closures); `ReadFromFile` panics→`recoverParse` fail-closed; uncapped uint64 height needs `>MaxInt64` guard before int64 cast; bundled `examples/*.ots` are the external `ots verify` oracle |
