...
| `cmd/wasm` (+ `cmd/wasm/verifyadapter`) | `learnings/cmd-wasm.md` | GOOS=js WASM entrypoint; untagged file in `package main` breaks `go build ./...` → split tagged `main.go` from a non-main `verifyadapter` subpackage; pure base64-Std adapter preserving proof/verify's three-way verdict at the marshaling boundary; `js.Value.Int()` truncation gotcha at the JS-call boundary (open hardening, no caller yet) |
