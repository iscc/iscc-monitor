...
| `internal/certificate` | `learnings/certificate.md` | realm-wide `GET /inclusion/{iscc_id}` cert; first caller of decode→resolve→ListHubs→SeqsForISCCID chain; fail-closed 200 verdicts + buffer-then-200; OPEN: §1 must gate on `seqs[0] < LastSize` + path-suffix vs stored `ISCC:`-prefixed id key mismatch |
