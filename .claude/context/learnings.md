...
| `internal/verifier` | `learnings/verifier.md` | Surface-C standalone Independent Verification page (`monitor.iscc.codes`); static SSR skeleton NOT mounted in buildMux (different origin); pure stdlib leaf, `/_ds/` paths are literals synced to `web.*`; chrome identity is `.codes` not an instance (correct mockup divergence); no-CDN body ban golden-tested; HONESTY GAP — mismatch alert renders unconditionally present-tense (filed `normal`) |
| `cmd/wasm` (+ `cmd/wasm/verifyadapter`) | `learnings/cmd-wasm.md` | GOOS=js WASM entrypoint; untagged file in `package main` breaks `go build ./...` → split tagged `main.go` from a non-main `verifyadapter` subpackage; pure base64-Std adapter preserving proof/verify's three-way verdict at the marshaling boundary; `safeIndex` integer guard CLOSED the `js.Value.Int()` truncation but is trapped in the tagged `main.go` → untested (move to `verifyadapter`); certificate is the first SSR caller (JSON data island + `/_ds/` loader, verified live) |
