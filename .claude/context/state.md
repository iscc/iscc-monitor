<!-- assessed-at: be2ed3ffe60def8c01781e2157b5e66fa857d27a -->
...
drains the ADR-0005 / follower-locality backlog. verify-for-me, the server-rendered dashboard, the
...
Incremental review of `4df69fb..HEAD` (HEAD `be2ed3f` on `develop`). The diff touches **two
production files** — `internal/store/checkpoints.go` (the new `AdvanceAccepted`) and a slimmed
`internal/follower/follower.go` — plus one new test (`checkpoints_test.go`). Verified from source:
`Store.AdvanceAccepted(ctx, CheckpointRecord)` is the repo's first `*sql.Tx` — it performs the
dedupe-insert (`ON CONFLICT(hub_id, tree_size, root) DO NOTHING`), the set-once coverage UPDATE
(guarded `monitored_since_size IS NULL`, ADR-0001), and the follow-cursor upsert against one
transaction (rollback-on-error via `defer _ = tx.Rollback()`, commit-last). The follower's
verified-advance path collapses the three caller-sequenced writes into the single
`st.AdvanceAccepted(ctx, rec)` call (follower.go:221); the direct `RecordCheckpoint(ctx, rec)` call is
gone (verified by grep — exit 1). The three legacy methods stay public (still seed helpers in
`*_test.go` and the freeze-path `RecordCheckpoint`). `go list -deps ./internal/store` confirms store
stays a leaf (no `net/http` / `internal/logclient` / `internal/follower` edge). Latest `review`
handoff (2026-06-21, "Collapse accepted-checkpoint advancement into one store-owned transaction
`AdvanceAccepted`") is **PASS / CONTINUE**: `mise run check` green (15 packages `ok`, `gofmt -l .`
clean), `TestAdvanceAccepted` reviewer-mutation-proven non-vacuous (drop coverage `IS NULL` guard →
case fails; no-op the cursor upsert → case fails), oracle gate correctly N/A (plain transactional SQL)
but the verified-advance flow re-run uncached, gate-integrity scan clean, scope-clean (2 production
files + 1 test). **CI green at HEAD `be2ed3f`** (develop run 27898839659, `conclusion: success`).

**Branch note:** active work happens on `develop` (HEAD `be2ed3f`, tree clean, in sync with
`origin/develop`); a human merges `develop`→`main`. The `main` branch lags at `c59d380`.

## M1 — Read-only Monitor
**Status**: met — carried forward; this slice did not touch the verify/consistency path. The prior
slice's pure `logclient.CheckConsistency` (shrink/fork/equivocation verdict) is unchanged. All Verify
...
- **Test totals at HEAD**: **208 `func Test`** across `cmd/` + `internal/` (+1), **48** `_test.go`
  files (unchanged) — this slice added `TestAdvanceAccepted` to the existing
...
- **All three triggers WIRED + golden-tested**: shrink → fork → equivocation, evaluated inside
  `logclient.CheckConsistency` (`checkconsistency.go`). The growing-pair equivocation builds the
  RFC-6962 consistency proof from the LOCAL mirror (`ConsistencyProofFromTiles` over the injected
  fetch) → freeze on `true` (ADR-0006). **Frozen hubs are evidence-only** on clean re-polls; fork
  re-detection on a frozen hub re-records evidence without re-alerting, proven through a real second
  `PollHub`.
...
- `store/*.go` — `modernc.org/sqlite`, ADR-0005/0007 single-writer discipline (WAL,
  ...
  `follow_state`, `ots`). `schema.sql` byte-unchanged this slice.
...
WIRED on every verified non-frozen poll; inclusion cross-check conformance-tested over the real
verified mirror), and the served proof surface is complete: all three computed proofs — `inclusion`,
`consistency`, `entries` — served from the local mirror, never re-hitting the hub. This slice did not
touch M2's proof or mirror logic.
...
- **computed inclusion/consistency/entries proofs (BUILT + WIRED):** `internal/proofserve/handler.go`
  ...
  shaped like the hub's `IsccLogInclusionProof` (inclusion) and `application/octet-stream`
  schema-agnostic record bytes (entries, ADR-0008).
...
in source — grep clean). The WASM-shared verifier seam continues to ride on `internal/didweb` (the
WASM-pure parser, not the networked resolver); the store-free `logclient.CheckConsistency` could share
a WASM seam later.
...
- **CI configured and passing.** `.github/workflows/ci.yml` runs the inlined `mise run check` gate (`go
  ...
  HEAD `be2ed3f`: `conclusion: success`** (run 27898839659).
- Latest `review` handoff (2026-06-21, **PASS / CONTINUE**) records `mise run check` green (15 packages
  `ok`, `go vet`/`gofmt -l .` clean), `TestAdvanceAccepted` reviewer-mutation-proven non-vacuous
  (coverage `IS NULL` guard + cursor upsert both fail when broken), oracle gate correctly N/A (plain
  transactional SQL) with the verified-advance flow re-run uncached, `go list -deps ./internal/store`
  confirms no `net/http`/logclient/follower edge (store stays a leaf), gate-integrity scan clean,
  scope-clean (2 production files + 1 test).
- **No open `critical` issue.**
- **Open `normal` issues (block DONE, do not block this slice's PASS)** — 1 in `issues.md` (was 2; the
  ADR-0005 "accepted-checkpoint advancement is three caller-sequenced store writes" issue is verified
  fixed by this slice and removed): tile writers require `width`, duplicating the tlog `p`-translation
  in the follower's `widthForP` copy (`internal/store/tiles.go` + `internal/follower/ingest.go`).
  **`low` (loop-skipped):** `cmd/notecheck`'s vestigial `out io.Writer` param.

## Next Milestone
**M1/M2 met; M3 in progress — the tlog-tiles mirror HTTP arc is complete, and accepted-state advancement
now commits as one store-owned `AdvanceAccepted` transaction. Drain the last `normal` issue, then begin
the verify-for-me / proof-surface cache arc.** No `critical` open, so feature work proceeds; the 1 open
`normal` issue blocks DONE and is weighed against the state→target gap.

Candidate order:
1. **`normal` backlog (ADR-0005 / locality)** — the tile-writer `p`-vocabulary unification: make
   `RecordTile`/`RecordEntryBundle` take `p uint8`, keep the `p→width` translation private to the
   store, and delete the follower's `widthForP` copy (`internal/follower/ingest.go`). This is the last
   open `normal` issue.
