<!-- assessed-at: 1d906d24c5d59e4de4efd41cadc1da948d8c3da5 -->

# Project State

## Status: IN_PROGRESS

## Phase: Certificate §4 / proof-bundle did:web `host:port` encode landed — gate green, pushed.
The certificate's two `did:web:` render sites (§4 `SigningKeyDID` + the proof-bundle `Hub.DID`) now route
through a local pure helper `didWeb(domain)` that `%3A`-encodes the FIRST colon, so a `host:port` hub names
the same host its key resolved from while a no-port domain round-trips byte-identical. Latest `review`
verdict is **PASS (loop CONTINUE)** and HEAD is **pushed** — `origin/develop` == HEAD, CI **success** at
HEAD. DONE not reached: the WASM "published"/signature-half and OTS Bitcoin-confirmed Verify criteria are
still open, and 8 `normal` issues stand.

Incremental review against assessed-at `3cf1878`. The `3cf1878..HEAD` diff touched exactly ONE production
file — `internal/certificate/handler.go` (+`didWeb` helper at `:117`, both DID sites rerouted at `:565`
bundle and `:886` §4) — plus its two test files (`handler_test.go`, `bundle_test.go`) and `.claude/context/*`
+ `learnings/certificate.md` docs. All M1/M2/M3/M-UI/WASM/OTS production source outside `internal/certificate`
is untouched — those sections carry forward met/open as before. **§4 + bundle `host:port` DID `normal`
CLOSED** (verified this assessment): `didWeb` exists at `handler.go:117` as
`"did:web:" + strings.Replace(domain, ":", "%3A", 1)`, both render sites route through it, and the two
mutation-proven tests (`TestCertificateSigningKeyDIDPortEncoded`, `TestCertificateProofBundleDIDPortEncoded`)
plus the clean-domain regressions (`…DIDCleanDomain`, `TestCertificateProofBundle`) are present. `git status`
clean, `origin/develop` == HEAD (`1d906d2`), CI run 27947334651 `success` at HEAD.
...
- **Remaining Verify criteria:**
  ...
  - **WASM verifier: 1/1 open.** Source + artifact are closed for the id-binding half: the byte-pinned
    ...
    **Still OPEN on the milestone Verify** ("the verifier artifact … [published at its] published value"):
    the Pages deploy **does not run** — the Pages run at HEAD (27947334634) fails at `Configure Pages`
...
- **Last ~10 iterations: ~3 milestone-Verify-advancing / ~7 chrome·plumbing·hardening.** Recent arc:
  WASM id-binding bind → rebuild+re-pin verify.wasm → pin `go=1.26.4` + re-pin (artifact reproducible) →
  `safeStamp` OTS guard → **§4/bundle `host:port` did:web encode (this iteration, PASS)**. **DRIFT WATCH
  (amber):** the front-of-queue WASM "published" Verify has not closed for nine increments, but its
  remaining open half (the live Pages deploy) is **human-blocked, not code-blocked** — a workflow file
  cannot self-enable Pages, so the loop cannot autonomously close it. This iteration correctly *pivoted
  off* the human-blocked WASM criterion to close a code-closable certificate `normal` (the right move per
  last assessment's guidance). Remaining code-closable productive targets: the WASM signature-half trust
  gap (design-first / STOP-candidate), the dossier WASM caller, and the remaining certificate/registry
  `normal`s (§5 digest-binding, `hubDomain` ForceQuery, tier-2 honesty copy).

## M1 — Read-only Monitor
**Status**: **met** — carried forward; no M1 source touched by the `3cf1878..HEAD` diff (confined to
`internal/certificate/*` + context/learnings docs). All M1 Verify criteria remain satisfied:
...
The only M-UI surface touched by this diff was `internal/certificate` — and only the rendered `did:web:`
STRING VALUE for a `host:port` hub (no testnet/live fixture produces one), so the served certificate page
is byte-identical for clean domains; no named-region / layout / affordance change. All six certificate
clauses (§1–§6) + both anchor panels + badge + DS shell + `/` index + log browser + hub dossier (with the
shared chrome masthead + `← Realm index` back-link) + frozen Exhibit + record list + single-record page +
...
- **Residual `normal` notes (NOT fixed):** certificate tier-2 honesty header overstates "this browser
  ...
  §6 rows omit the per-record `· at` timestamp. (The `host:port` DID encode `normal` is now CLOSED.)
...
- **WASM (carried):** the id-binding half of the verifier-scope trust gap is closed IN SOURCE and ARTIFACT,
  ...
  `failure` at `Configure Pages` at HEAD), needs the one-time human repo-Settings step. **Carried `normal`
  defects:** the verifier core does NO checkpoint-signature / did:web check (a malicious cross-origin
  monitor can render green `verified`; success copy overstates a key check that never runs) — design-first
  remainder; Surface-C `readTarget` accepts opaque-scheme monitor forms; the Pages
...
## Quality gates
**Status**: **GREEN and PUSHED.** CI `success` at HEAD (`1d906d2`) == `origin/develop` (run 27947334651);
...
- `go.mod` present (`module github.com/iscc/iscc-monitor`, `go 1.26.1` language directive; build toolchain
  ...
  at HEAD (all 27 packages build + vet + test; `gofmt -l .` empty); both DID sites are mutation-proven
  (reverting `didWeb(data.Domain)` → `"did:web:" + data.Domain` FAILS the §4 and bundle port-encode tests).
- **Latest `review` verdict: PASS (loop CONTINUE)** for the §4/bundle `host:port` did:web encode. The
  reviewer confirmed the diff is exactly `handler.go` + two test files (+ docs), both sites mutation-proven,
  the clean-domain regression intact, no remaining unencoded `"did:web:" + domain` concat sites in
  production, the round-trip correct against the resolver (`didweb.DocumentURL`), the conformance/oracle
  gate N/A (render-string fix only, no signature/Merkle/proof code), no gate-circumvention, and Codex
  corroborated clean.
- **CI**: `.github/workflows/ci.yml` runs the inlined `mise run check` + the `cmd/notecheck` oracle on
  push/PR. Remote `origin` = `github.com/iscc/iscc-monitor.git`, branch `develop`. HEAD (`1d906d2`) is
  pushed; CI run 27947334651 is `success` at HEAD.
- **Pages**: `.github/workflows/pages.yml` runs on push to develop; the latest run (27947334634) at HEAD is
...
- **Open issues: 0 critical, 8 normal, 10 low** (the lone "critical" grep hit is the format-template legend
  on issues.md:9, excluded). DONE requires 0 critical AND 0 normal, so the loop stays CONTINUE. Tally moved
  from "0 critical / 9 normal" last assessment to "0 critical / 8 normal" — the §4/bundle `host:port` DID
  encode `normal` closed; no new normal filed. The 8 normal: certificate §5 digest binding, the `hubDomain`
  ForceQuery gap, certificate §6 timestamp, the certificate tier-2 no-JS honesty-copy overstatement, the
...
## Next Milestone
**CI green and pushed; the code-closable certificate did:web `normal` just closed. Resume the front-of-queue
...
2. **Remaining code-closable certificate/registry `normal`s** (handoff-named order): the `hubDomain`
   ForceQuery fail-open in `internal/registry/registry.go` (add `|| u.ForceQuery` to the line-188 reject);
   the §5 OTS-digest-binding `bytes.Equal` gap; the tier-2 honesty-copy overstatement (`cert.html:465`).
...
Subsequent: the OTS "upgrades to Bitcoin-confirmed" half (offline-unprovable); the §6 `· at` timestamp when
the projection gains a record-timestamp column; the named-region + `←` back-link parity pass across the
remaining SSR surfaces; the M-UI exit visual-pass + human sign-off (ADR-0012).
