<!-- assessed-at: ed4668a802aaec93a1cc839b6672f309621cfa4b -->

# Project State

## Status: IN_PROGRESS

## Phase: M1 complete + CI-gated & green — M2 (Aggregator) is the active milestone. Seven M2 prerequisite slices have now landed (the tile-sourced **inclusion** proof builder is the newest), but M2's Verify bar — live tile ingestion, the inclusion cross-check vs the hub's own proof, `iscc_index`, and store-served proofs — remains not-started.

Since the prior assessment (`64b3339`) the **only source-relevant change is one additive seam**:
`internal/logclient/proofbuilder.go` gained the exported `InclusionProofFromTiles(ctx, fetch
TileFetcher, index, size uint64) ([][]byte, error)` plus its golden test
`inclusionproof_test.go` (`14ac42b`, reviewed PASS in `ed4668a`). `git diff 64b3339..HEAD --stat`
touches no other `.go`/`go.mod`/`go.sum`/`schema.sql` byte (only those two logclient files + context
md). The new builder is a faithful sibling mirror of `ConsistencyProofFromTiles` with the two intended
swaps (`proof.Inclusion(index, size)` + `size` as `getNode`'s logSize); the RFC-6962 oracle gate was
mutation-proven non-vacuous by review. It is an **unwired export seam** (`grep` confirms it appears
only in its own definition + test). The project stays off DONE because M2 → OTS are not yet built.
...
- **Verified incrementally** from `64b3339`. The diff `64b3339..HEAD` touches no
  Go/`go.mod`/`go.sum`/`schema.sql` byte outside `internal/logclient/proofbuilder.go` (+inclusion
  builder) and the new `inclusionproof_test.go`. Every cmd/iscc-monitor / follower / logclient (sans the
  one new builder) / store / didweb / tiles / metrics / metricshttp / registry / config source +
  `cmd/notecheck` + `schema.sql` is carried forward byte-unchanged.
- **Test totals re-grepped at HEAD**: 9 (didweb) + 36 (logclient, +3 from the inclusion test) + 48
  (store) + 14 (follower) + 3 (registry) + 4 (config) + 5 (tiles) + 6 (metrics) + 1 (metricshttp) + 1
  (cmd/iscc-monitor) + 3 (cmd/notecheck) = **130 `func Test`** across **11 packages** (**31** `_test.go`
  files).

- **`cmd/notecheck` — fully-independent signature-parity oracle (CI-gated)**: reads `--vkey` +
  checkpoint text on stdin, runs `transparency-dev/formats/note.NewVerifier` + `golang.org/x/mod/
  sumdb/note.Open`, applies the strict reject (`len(n.Sigs)==0 || len(n.UnverifiedSigs)!=0`) mirroring
  ...
  in-process `RunFsck` self-check. Three tests (golden accept + corrupted-body reject + bad-vkey
  exit-2). **CI shells the built binary out** against `testdata/live/sb0.iscc.id_checkpoint`, asserting
  the exact `OK sb0.iscc.id/log` accept AND a one-char-flipped-signature reject (exit 1) — review
  mutation-proved the reject guard catches a green-but-wrong always-accept stub.
...
- **equivocation trigger WIRED, production-dormant** (unchanged, re-verified): `checkConsistency`
  evaluates shrink → fork → equivocation; the growing-pair branch builds the RFC-6962 consistency proof
  from the LOCAL mirror and on `true` → freeze, with a narrow proof-build-error skip (ADR-0006 guard).
  Golden-tested + mutation-proven across the 256-leaf tile boundary. **Dormant on the live path until
  M2's tile-ingestion writer lands** — `follower.go` confirms `PollHub` never writes tiles (the branch
  "only becomes load-bearing once M2 mirrors tiles", per its own comment).
...
- **Fixtures**: `testdata/live/` holds **only the two checkpoints** (`sb0.iscc.id_checkpoint`,
  ...
  `LeafHashes`, `RunFsck`, `notecheck`, and both proof-builder golden tests use these checkpoints or
  synthesize in-process `testonly.Tree` inputs; real on-disk tile/entry-bundle fixtures are still
  needed for the inclusion cross-check vs the hub's `IsccLogInclusionProof` and a 256-crossing live
  `fsck` case (M2). **Known stale-fixture drift, still not acted on:** the `sb1.amlet.id_did.json`
...
- **Reuse imports wired**: `golang.org/x/mod/sumdb/note`, `modernc.org/sqlite`, `transparency-dev/merkle
  v0.0.2` (`rfc6962`, `proof` — now `proof.Inclusion` AND `proof.Consistency`), `transparency-dev/
  tessera v1.0.2` (`tiles/layout.go`; both proof builders; `leafhasher.go`; `tessera/fsck` in
...
## M2 — Aggregator
**Status**: not started — but **seven prerequisite slices have landed** (all carried forward):
...
- `internal/logclient/proofbuilder.go` — **`InclusionProofFromTiles(ctx, fetch, index, size)` (NEW
  this iteration)**: tile-sourced RFC-6962 inclusion proof, golden-tested over a 300-leaf
  `testonly.Tree` boundary fixture (indices `{0,5,200,255,256,260,299}` byte-equal
  `tree.InclusionProof` AND verify via `proof.VerifyInclusion`); review mutation-proved the oracle gate
  non-vacuous two ways. **Unwired export seam** — first caller is the M2 inclusion cross-check below;
...
- `cmd/notecheck` — the **fully-independent** external signature-parity oracle, compiling in-repo AND
  shelled out in CI on every push, so the trust root is gated, not just locally reproducible.

What remains for M2's Verify bar (all not-started): the **inclusion cross-check** — assert
`InclusionProofFromTiles` byte-equals a hub's real `evidence.IsccLogInclusionProof` for sampled
`iscc_id`s (now blocked only on artifacts, not on the builder: needs captured `IsccLogInclusionProof` +
tile + entry-bundle fixtures in `testdata/live/`, plus a tile source feeding `SQLiteFetcher.ReadTile`);
the **live tile-ingestion writer** (make `PollHub` mirror real tiles/bundles — also un-dormants the
equivocation branch and gives `RunFsck` its first production `fsck` root-rebuild caller); the
`iscc_index` projection writer (schema-agnostic, `iscc_id → seq` one-to-many); and
...
- **CI is configured and passing.** `.github/workflows/ci.yml` runs one `ubuntu-latest` /
  ...
  run 27891066038). Remote `origin` configured (`github.com/iscc/iscc-monitor`); working branch
  `develop`; tree clean at HEAD `ed4668a`.
- Latest `review` handoff (2026-06-21, "Port `InclusionProofFromTiles` — the tile-sourced RFC-6962
  inclusion proof builder", verdict **PASS / CONTINUE**) records `mise run check` green (all 11 packages
  `ok`, `go vet`/`gofmt -l .` clean) with the RFC-6962 inclusion oracle mutation-proven by the reviewer
  (two reverted mutations both failed the golden). Gate-integrity + scope-discipline scans clean.
...
Verify bar. With the inclusion proof **builder** now landed and oracle-proven, the next slice is the
**live tile-ingestion writer** — the natural unblocker that both pairs the inclusion **cross-check**
(builder vs the hub's real `evidence.IsccLogInclusionProof`) and gives `RunFsck`/the `fsck` root-rebuild
its first production caller over `SQLiteFetcher`; both want the same real tile + entry-bundle fixtures.

Candidate order:
1. **Live tile-ingestion writer** — make `PollHub` mirror real tiles/bundles, which un-dormants the
   wired equivocation branch on the live path AND feeds `SQLiteFetcher.ReadTile` for proof nodes. The
   unblocker for both crypto cross-checks below.
2. **Inclusion cross-check** — assert `InclusionProofFromTiles` byte-equals the hub's own
   ...
   `IsccLogInclusionProof` + tile + entry-bundle fixtures in `testdata/live/`).
