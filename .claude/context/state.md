<!-- assessed-at: 64b34f927ed2a6d132df25339ea01a6c51a85870 -->

# Project State

## Status: IN_PROGRESS

## Phase: Log-browser record-list `Logged` column landed (issue:214 record-list named region); gate green + pushed.
The log-browser record list (`GET /<domain>/log/records`) now renders the mockup's `Logged` column —
`RecordRow.NoteTimestamp` rendered verbatim RFC-3339 (ADR-0008, never re-formatted) with an honest
`&mdash;` fallback for the NULL-timestamp case, plus a `Seq · ISCC-ID · Logged` header row. Latest
`review` verdict is **PASS (loop CONTINUE)**: mutation-proven non-vacuous, Codex clean, agent-browser
visual pass confirms the named region matches the mockup with no new delta. DONE not reached: WASM
"published" + signature halves and OTS Bitcoin-confirmed stay open, and 5 `normal` issues stand.

Incremental review against assessed-at `d9dd9ee`. The `d9dd9ee..HEAD` diff touched exactly ONE
production file (`internal/proofserve/records.html` — the `Logged` column markup + CSS) plus its test
(`records_test.go` — new `TestRecordsRendersLoggedColumn`) and one learnings doc; the rest is
`.claude/context/*`. Verified: the name-only diff over every trust-root glob (`internal/proof/`,
`logclient`, `didweb`, `follower`, `cmd/wasm`, `verifier`, `internal/web/`, `ots`, `certificate`,
`dossier`, `go.mod`, `go.sum`, `schema.sql`, `internal/store/`) is **empty** — no signature / RFC-6962
/ Merkle / store / WASM / OTS source changed. All M1/M2/M3/WASM/OTS sections carry forward met/open as
before. `git status` clean; HEAD (`64b34f9`) == `origin/develop` (0 ahead / 0 behind).
...
- **Remaining Verify criteria:**
  ...
  - **WASM verifier: 1/1 open.** The id-binding half is closed in source AND artifact (byte-pinned
    ...
    (27955840698) is `failure` at `Configure Pages` (Pages not enabled / not set to "GitHub Actions"
...
- **Last ~10 iterations: ~1 milestone-Verify-advancing / ~9 chrome·plumbing·hardening.** Recent arc:
  §6 `· at` render → `/` realm-index Checkpoint/Anchor columns (closed sub-delta 3 of issue:214) →
  **log-browser `Logged` column (this iteration, PASS — record-list named region).** **DRIFT WATCH
  (amber):** no *milestone-level* Verify criterion has closed in ~17 increments (the last were the WASM
  id-binding slices ~12 iters back), but the loop is legitimately draining the code-closable M-UI
  named-region backlog rather than re-polishing met surfaces. The front-of-queue WASM "published" half
  stays **human-blocked, not code-blocked** (a workflow file cannot self-enable Pages), so the loop
  correctly pivots off it. The remaining pure-code slices are now nearly exhausted: the sibling record-list
  `Type` column / per-row type badge (the next named-region step, completes the 4-column
  `Seq · Type · ISCC-ID · Logged` head) is the cheapest pickable; after that the named-region work needs
  store history that does not exist (recent-declarers footer) or `internal/config` wiring (instance
  identity), and the rest is design-first (WASM signature half, per-hub Anchor honesty, DB migration) or
  human-blocked (Pages, custom domain). After the `Type` column, flag a STOP/design candidate rather than
  re-polishing met surfaces.

## M1 — Read-only Monitor
**Status**: **met** — carried forward. The `d9dd9ee..HEAD` diff touched no signature / RFC-6962 / Merkle
/ `proof` / `didweb` / `logclient` / `follower` file (the only production edit is the `records.html`
`Logged` column render). All M1 Verify criteria remain satisfied: `origin`/`vkey` golden;
...
This diff landed the log-browser record-list `Logged` column (verbatim RFC-3339 `NoteTimestamp` with a
`&mdash;` NULL fallback + the `Seq · ISCC-ID · Logged` header), advancing the log-browser record-list
named region. All six certificate clauses (§1–§6) + both anchor panels + badge + DS shell + `/` index
(six-column ledger) + log browser + hub dossier (with shared chrome masthead + `← Realm index`
back-link) + frozen Exhibit + record list + single-record page + ISCC-IDv1 decoder + Hub-List resolver +
...
- **Still open (NOT critical, carried):** the named-region + `←` back-link parity pass is on the dossier
  ...
  instance-identity block + back-link chain on every surface); the record-list `Type` column / per-row
  type badge (the deferred sibling slice — completes the mockup's 4-column head) is the cheapest remaining
  code-only slice; the `/` sub-region deltas — config-driven instance identity/realm name (needs
  `internal/config` env wiring) and "recent declarers checked" footer (needs a recent-lookup history the
...
- **WASM (carried):** the id-binding half of the verifier-scope trust gap is closed IN SOURCE and
  ...
  (Pages `failure` at `Configure Pages` at HEAD, run 27955840698), needs the one-time human repo-Settings
...
## Quality gates
**Status**: **GREEN and PUSHED.** CI `success` at HEAD (`64b34f9`) == `origin/develop` (run
27955840696); the Pages PUBLISH workflow is `failure` at `Configure Pages` (human-step, not a code/gate
...
- `go.mod` present (`module github.com/iscc/iscc-monitor`, `go 1.26.1` language directive; build toolchain
  ...
  at HEAD (build + vet + `go test ./...`, all 28 packages ok; `gofmt -l .` empty); the `Logged` column is
  mutation-proven (deleting the `.record-logged` cell from `records.html` fails
  `TestRecordsRendersLoggedColumn`). The trust-root oracles are unaffected — name-only diff over the
  signature / RFC-6962 / Merkle / `proof` / `didweb` / store globs is empty (this is a pure HTML render of
  a persisted leaf read; oracle gate correctly N/A).
- **CI**: `.github/workflows/ci.yml` runs the inlined `mise run check` + the `cmd/notecheck` oracle on
  push/PR. Remote `origin` = `github.com/iscc/iscc-monitor.git`, branch `develop`. HEAD (`64b34f9`) is
  pushed (0 ahead / 0 behind upstream); CI run 27955840696 is `success` at HEAD.
- **Pages**: `.github/workflows/pages.yml` runs on push to develop; the latest run (27955840698) at HEAD
...
## Next Milestone
**CI green and pushed; the log-browser `Logged` column is closed. The cheapest remaining code-only slice
is the record-list `Type` column; after that the backlog is design-first or human-blocked.** In order:
1. **Render the record-list `Type` column / per-row type badge (`declaration`/`deletion`/`unknown`) in
   `internal/proofserve`** — completes the mockup's 4-column `Seq · Type · ISCC-ID · Logged` head. Needs a
   per-row `recordKind(NoteSchema)` precompute mapped into a small row VM in `serveRecords` (`handler.go`),
   reusing `recordKind` + the full-URI `schemaDeclaration`/`schemaDeletion` constants from `record.go`.
   Seed the test with a HARDCODED literal schema URI (constant-vs-constant goes vacuous — see the open
   `low` on `record_test.go`). Stays code-only; store stays a leaf. The cheapest pickable slice; prefer it
...
remaining SSR surfaces; the no-on-disk-DB-migration mechanism (the project's first migration framework);
the M-UI exit visual-pass + human sign-off (ADR-0012).
